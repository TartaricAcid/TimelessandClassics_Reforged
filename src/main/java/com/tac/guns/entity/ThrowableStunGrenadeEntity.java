...
public class ThrowableStunGrenadeEntity extends ThrowableGrenadeEntity {
    ...
    public static void blindMobs(LivingChangeTargetEvent event) {
        if (Config.COMMON.stunGrenades.blind.blindMobs.get() && event.getNewTarget() != null && event.getEntity() instanceof Mob && event.getEntity().hasEffect(ModEffects.BLINDED.get())) {
