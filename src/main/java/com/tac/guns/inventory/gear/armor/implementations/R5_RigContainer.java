...
public class R5_RigContainer extends AbstractContainerMenu implements IRigContainer {
    public final static int ROW_NUM = 5; //swap per row count

    public R5_RigContainer(int windowId, Inventory inv, ItemStack item) {
        super(ModContainers.ARMOR_R5.get(), windowId); // Swap per row count
    ...
    public R5_RigContainer(int windowId, Inventory inv) {
        super(ModContainers.ARMOR_R5.get(), windowId);  // Swap per row count
    ...
    private static boolean isAmmo(ItemStack stack, ResourceLocation id) {
        return stack != null && ForgeRegistries.ITEMS.getKey(stack.getItem()).equals(id);
    }
