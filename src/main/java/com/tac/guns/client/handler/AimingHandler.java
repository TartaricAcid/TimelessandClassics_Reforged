...
public class AimingHandler {
    ...
    @SubscribeEvent
    public void onLocalPlayerLoggedOut(ClientPlayerNetworkEvent.LoggingOut event) {
                    ...
                    if (modifiedGun.getModules().getZoom() != null) {
                        ...
                        if (scope != null) {
                            if (!Config.COMMON.gameplay.realisticLowPowerFovHandling.get() || (scope.getAdditionalZoom().getZoomMultiple() > 1 && Config.COMMON.gameplay.realisticLowPowerFovHandling.get()) || gunItem.isIntegratedOptic()) {
                                newFov = (float) MathUtil.magnificationToFovMultiplier(scope.getAdditionalZoom().getZoomMultiple(), Minecraft.getInstance().options.fov().get());
    ...
    @SubscribeEvent
    public void captureFovAndModifyMouseSensitivity(ViewportEvent.ComputeFov event) {
        if (!isRenderingHand) {
            Minecraft mc = Minecraft.getInstance();
            double modifier = MathUtil.fovToMagnification(event.getFOV(), mc.options.fov().get());
            ((MouseSensitivityModifier) mc.mouseHandler).setSensitivity(mc.options.sensitivity().get() / modifier);
        }
    }
