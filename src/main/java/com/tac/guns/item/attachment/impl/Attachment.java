...
public abstract class Attachment {
    ...
    public static void addInformationEvent(ItemTooltipEvent event) {
        ...
        if (stack.getItem() instanceof IAttachment<?>) {
            ...
            if (perks != null && perks.size() > 0) {
                event.getToolTip().add(Component.translatable("perk.tac.title").withStyle(ChatFormatting.GOLD, ChatFormatting.BOLD));
            ...
            if (outputSound > inputSound) {
                addPerk(negativePerks, "perk.tac.fire_volume.negative", Component.translatable("+" + String.valueOf((1.0F - Math.round(outputSound)) * 100) + "% Volume").withStyle(ChatFormatting.RED));
            } else if (outputSound < inputSound) {
                addPerk(negativePerks, "perk.tac.fire_volume.negative", Component.translatable("" + String.valueOf((1.0F - Math.round(outputSound)) * 100) + "% Volume").withStyle(ChatFormatting.GREEN));
                //addPerk(positivePerks, "perk.tac.fire_volume.positive", ChatFormatting.GREEN, "-" + String.valueOf((1.0F - outputSound) * 100) + Component.translatable("perk.tac.vol"));
            }*/

            /* Test for silenced */
            for (IGunModifier modifier : modifiers) {
                if (modifier.silencedFire()) {
                    addPerkP(positivePerks, "perk.tac.silenced.positive", Component.translatable("perk.tac.silencedv2").withStyle(ChatFormatting.GREEN));
                    break;
                }
            }
            ...
            if (outputRadius > inputRadius) {
                addPerkN(negativePerks, "perk.tac.sound_radius.negative", Component.translatable("-")
                        .append(Component.translatable("perk.tac.sound_radiusv2", Math.round(outputRadius)).withStyle(ChatFormatting.RED)));
            } else if (outputRadius < inputRadius) {
                addPerkP(positivePerks, "perk.tac.sound_radius.positive", Component.translatable("+")
                        .append(Component.translatable("perk.tac.sound_radiusv2", Math.round(outputRadius)).withStyle(ChatFormatting.GREEN)));
            }
            ...
            if (outputDamage > inputDamage) {
                addPerkP(positivePerks, "perk.tac.modified_damage.positive", Component.translatable("perk.tac.modified_damage.positivev2", outputDamage).withStyle(ChatFormatting.GREEN));
            } else if (outputDamage < inputDamage) {
                addPerkN(positivePerks, "perk.tac.modified_damage.negative", Component.translatable("perk.tac.modified_damage.negativev2", outputDamage).withStyle(ChatFormatting.RED));
            }
            ...
            if (outputSpeed > inputSpeed) {
                addPerkP(positivePerks, "perk.tac.projectile_speed.positive", Component.translatable("perk.tac.projectile_speed.positivev2", Math.round((10.0F - outputSpeed) * 10) + "%"));
            } else if (outputSpeed < inputSpeed) {
                addPerkN(negativePerks, "perk.tac.projectile_speed.negative", Component.translatable("perk.tac.projectile_speed.negativev2", Math.round((10.0F - outputSpeed) * 10) + "%"));
            }
            ...
            if (outputSpread > inputSpread) {
                addPerkN(negativePerks, "perk.tac.projectile_spread.negative", Component.translatable("perk.tac.projectile_spread.negativev2", Math.round((10.0F - outputSpread) * 10) + "%").withStyle(ChatFormatting.RED));
            } else if (outputSpread < inputSpread) {
                addPerkP(positivePerks, "perk.tac.projectile_spread.positive", Component.translatable("perk.tac.projectile_spread.positivev2", Math.round((10.0F - outputSpread) * 10) + "%").withStyle(ChatFormatting.GREEN));
            }
            ...
            if (positivePerks.size() > 0) {
                event.getToolTip().add(Component.translatable("perk.tac.title").withStyle(ChatFormatting.GRAY, ChatFormatting.BOLD));
    ...
    private static void addPerk(List<Component> components, String id, Object... params) {
        //ChatFormatting format,   components.add(Component.translatable("perk.tac.entry.negative", Component.translatable(id, params).withStyle(format)));
        components.add(Component.translatable("perk.tac.entry.negative", Component.translatable(id, params).withStyle(ChatFormatting.AQUA)));
    }

    private static void addPerkP(List<Component> components, String id, Object... params) {
        //ChatFormatting format,   components.add(Component.translatable("perk.tac.entry.negative", Component.translatable(id, params).withStyle(format)));
        components.add(Component.translatable(id, params).withStyle(ChatFormatting.GREEN));
    }

    private static void addPerkN(List<Component> components, String id, Object... params) {
        //ChatFormatting format,   components.add(Component.translatable("perk.tac.entry.negative", Component.translatable(id, params).withStyle(format)));
        components.add(Component.translatable(id, params).withStyle(ChatFormatting.RED));
    }
