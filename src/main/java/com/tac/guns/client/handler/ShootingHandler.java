            ...
            if (Keys.PULL_TRIGGER.isDown()) {
                ...
                if (emptyCheckCountDown > emptyCheckCoolDown) {
                    if (magError(player, heldItem)) {
                        ...
                        player.displayClientMessage(Component.translatable("info.tac.mag_error").withStyle(ChatFormatting.UNDERLINE).withStyle(ChatFormatting.BOLD).withStyle(ChatFormatting.RED), true);
                ...
                if (emptyCheckCountDown > emptyCheckCoolDown) {
                    if (!(heldItem.getTag().getInt("AmmoCount") > 0)) {
                        emptyCheckCountDown = 0;
                        player.displayClientMessage(Component.translatable("info.tac.out_of_ammo").withStyle(ChatFormatting.UNDERLINE).withStyle(ChatFormatting.BOLD).withStyle(ChatFormatting.RED), true);
                        PacketHandler.getPlayChannel().sendToServer(new MessageEmptyMag());
                    }
                }
            }
