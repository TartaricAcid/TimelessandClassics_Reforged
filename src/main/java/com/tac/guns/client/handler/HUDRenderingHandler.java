    ...
    private void brightenScreen(boolean doNightVision) {
        ...
        if (doNightVision) {
            if (defaultGameGamma == 0)
                defaultGameGamma = Minecraft.getInstance().options.gamma().get();
            Minecraft.getInstance().options.gamma().set(200.0);
        } else {
            Minecraft.getInstance().options.gamma().set(defaultGameGamma);
        }
    }
    ...
    public void onOverlayRender(RenderGuiEvent.Post event) {
        ...
        if (ArmorInteractionHandler.get().isRepairing())//Replace with reload bar checker
        {
            ...
            stack.pushPose();
            {
                ...
                // stack.translate(0, 0, );
                stack.scale(2.1F * (1 - ArmorInteractionHandler.get().getRepairProgress(event.getPartialTick(), player)), 0.25F, 0); // *21F
            ...
            stack.pushPose();
            {
                ...
                if (player.getMainHandItem().getTag() != null) {
                    ...
                    if (player.getMainHandItem().getTag().getInt("AmmoCount") <= gun.getReloads().getMaxAmmo() / 4 && this.ammoReserveCount <= gun.getReloads().getMaxAmmo()) {
                        currentAmmo = byPaddingZeros(ammo).append(Component.translatable("" + ammo)).withStyle(ChatFormatting.RED);
                        reserveAmmo =
                                byPaddingZeros(this.ammoReserveCount > 10000 ? 10000 : this.ammoReserveCount).append(Component.translatable("" + (this.ammoReserveCount > 10000 ? 9999 : this.ammoReserveCount))).withStyle(ChatFormatting.RED);
                    } else if (this.ammoReserveCount <= gun.getReloads().getMaxAmmo()) {
                        currentAmmo = byPaddingZeros(ammo).append(Component.translatable("" + ammo).withStyle(ChatFormatting.WHITE));
                        reserveAmmo = byPaddingZeros(this.ammoReserveCount > 10000 ? 10000 : this.ammoReserveCount).append(Component.translatable("" + (this.ammoReserveCount > 10000 ? 9999 : this.ammoReserveCount))).withStyle(ChatFormatting.RED);
                    } else if (player.getMainHandItem().getTag().getInt("AmmoCount") <= gun.getReloads().getMaxAmmo() / 4) {
                        currentAmmo = byPaddingZeros(ammo).append(Component.translatable("" + ammo)).withStyle(ChatFormatting.RED);
                        reserveAmmo = byPaddingZeros(this.ammoReserveCount > 10000 ? 10000 : this.ammoReserveCount).append(Component.translatable("" + (this.ammoReserveCount > 10000 ? 9999 : this.ammoReserveCount))).withStyle(ChatFormatting.GRAY);
                    } else {
                        currentAmmo = byPaddingZeros(ammo).append(Component.translatable("" + ammo).withStyle(ChatFormatting.WHITE));
                        reserveAmmo = byPaddingZeros(this.ammoReserveCount > 10000 ? 10000 : this.ammoReserveCount).append(Component.translatable("" + (this.ammoReserveCount > 10000 ? 9999 : this.ammoReserveCount))).withStyle(ChatFormatting.GRAY);
                    }
