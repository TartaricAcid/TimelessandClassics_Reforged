        ...
        for (int i = 0; i < IAttachment.Type.values().length; i++) {
            if (RenderUtil.isMouseWithin(mouseX, mouseY, startX + 7, startY + 16 + i * 18, 18, 18)) {
                ...
                if (!this.menu.getSlot(i).isActive()) {
                    this.renderComponentTooltip(matrixStack, Arrays.asList(Component.translatable("slot.tac.attachment." + type.getTranslationKey()), Component.translatable("slot.tac.attachment.not_applicable")), mouseX, mouseY);
                } else if (this.weaponInventory.getItem(i).isEmpty()) {

                    this.renderComponentTooltip(matrixStack, Collections.singletonList(Component.translatable("slot.tac.attachment." + type.getTranslationKey())), mouseX, mouseY);
                }
            }
        }
    }
