...
public class TimelessGunItem extends GunItem {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level worldIn, List<Component> tooltip, TooltipFlag flag) {
        ...
        tooltip.add((Component.translatable("info.tac.damage", ChatFormatting.GOLD + ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(additionalDamage) + additionalDamageText)).withStyle(ChatFormatting.DARK_GRAY));
        if (tagCompound != null) {
            if (tagCompound.getBoolean("IgnoreAmmo")) {
                tooltip.add((Component.translatable("info.tac.ignore_ammo")).withStyle(ChatFormatting.AQUA));
            } else {
                int ammoCount = tagCompound.getInt("AmmoCount");
                tooltip.add((Component.translatable("info.tac.ammo", ChatFormatting.GOLD.toString() + ammoCount + "/" + GunModifierHelper.getAmmoCapacity(stack, modifiedGun))).withStyle(ChatFormatting.DARK_GRAY));
            }
        }
        ...
        if (!isShift) {
            ...
            String text = (Component.keybind("key.tac.more_info_hold")).getString().toUpperCase(Locale.ENGLISH);
            tooltip.add((Component.translatable("info.tac.more_info_gunitem", text)).withStyle(ChatFormatting.YELLOW));
        }
        if (isShift) {
            ...
            if (tagCompound != null) {
                ...
                tooltip.add((Component.translatable("info.tac.armorPen", Component.translatable(String.format("%.1f", armorPen) + "%").withStyle(ChatFormatting.RED)).withStyle(ChatFormatting.DARK_AQUA)));
                ...
                tooltip.add((Component.translatable("info.tac.headDamageModifier", Component.translatable(String.format("%d", headDamgeModifier) + "%").withStyle(ChatFormatting.RED)).withStyle(ChatFormatting.DARK_AQUA)));
                ...
                tooltip.add((Component.translatable("info.tac.current_level").append(Component.translatable(" " + tagCompound.getInt("level") + " : " + String.format("%.2f", percentageToNextLevel) + "%")))
                        .withStyle(ChatFormatting.GRAY).withStyle(ChatFormatting.BOLD));
            }

            tooltip.add((Component.translatable("info.tac.attachment_help", Keys.ATTACHMENTS.getKey().getDisplayName().getString().toUpperCase(Locale.ENGLISH))).withStyle(ChatFormatting.YELLOW));
            ...
            if (gun.getGun().canAttachType(IAttachment.Type.IR_DEVICE))
                tooltip.add((Component.translatable("info.tac.irLaserEquip", Component.translatable("IrLaser").withStyle(ChatFormatting.BOLD)).withStyle(ChatFormatting.AQUA)));
        }
    }
