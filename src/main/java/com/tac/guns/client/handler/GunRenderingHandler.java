...
public class GunRenderingHandler {
    ...
    @SubscribeEvent
    public void onFovModifying(ViewportEvent.ComputeFov event) {
        Minecraft mc = Minecraft.getInstance();
    ...
    public void onRenderOverlay(RenderHandEvent event) {
        ...
        if (mc.options.bobView().get() && mc.getCameraEntity() instanceof Player) {
            ...
            float distanceWalked = -(playerentity.walkDist + deltaDistanceWalked * event.getPartialTick());
            float cameraYaw = Mth.lerp(event.getPartialTick(), playerentity.oBob, playerentity.bob);
            ...
            applyJumpingTransforms(matrixStack, event.getPartialTick());
        ...
        applyDelayedSwayTransforms(matrixStack, entity, event.getPartialTick());
        ...
        int blockLight = entity.isOnFire() ? 14 : entity.level.getBrightness(LightLayer.BLOCK, new BlockPos(entity.getEyePosition(event.getPartialTick())));
        if (blockLight > 14) blockLight = 14;
        int packedLight = LightTexture.pack(blockLight, entity.level.getBrightness(LightLayer.SKY, new BlockPos(entity.getEyePosition(event.getPartialTick()))));

        //this.applySprintingTransforms(heldItem, hand, matrixStack, event.getPartialTick());
        /* Applies recoil and reload rotations */
        this.applyRecoilTransforms(matrixStack, heldItem, modifiedGun);
        //if(!isAnimated) this.applyReloadTransforms(matrixStack, hand, event.getPartialTick(), heldItem);
    ...
    public void applyBobbingTransforms(PoseStack matrixStack, boolean convert, float effectMultiplier) {
        ...
        if (wSpeed > 0.094f)
            matrixStack.mulPose(Vector3f.XP.rotationDegrees(delta * effectMultiplier * 5f * sprintTransition));
    ...
    public void applyNoiseMovementTransform(PoseStack matrixStack) {
        ...
        matrixStack.translate(
                aimed_noiseX.getValue() / 2 * progress + noiseX.getValue() * (1 - progress),
                aimed_noiseY.getValue() * progress + noiseY.getValue() * (1 - progress) + additionNoiseY.getValue(),
                0
        );
        matrixStack.mulPose(Vector3f.YP.rotationDegrees((float) (aimed_noiseRotationY.getValue() * 0.405f * progress + noiseRotationY.getValue() * (1 - progress))));
        matrixStack.mulPose(Vector3f.ZP.rotationDegrees((float) (aimed_noiseRotationY.getValue() * 0.925 * progress)));
    }
    ...
    public void onRenderHeldItem(RenderItemEvent.Held.Pre event) {
        InteractionHand hand = Minecraft.getInstance().options.mainHand().get() == HumanoidArm.RIGHT ? event.getHandSide() == HumanoidArm.RIGHT ? InteractionHand.MAIN_HAND : InteractionHand.OFF_HAND : event.getHandSide() == HumanoidArm.LEFT ? InteractionHand.MAIN_HAND : InteractionHand.OFF_HAND;
